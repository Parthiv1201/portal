    ...
    <div ng-app="RegistrationForm" ng-controller="Registration">
      ...
      <div class="inner">
        <form name="myForm" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post" novalidate>
          <span>Mobile No. :- </span><input type="tel" name="mob" placeholder="XXXXXXXXXX" ng-model="mobile_no" ng-pattern="/^[0-9]{1,10}$/" maxlength="10" ng-minlength="10" ng-required="true"><br>
          ...
          <span>Password :- </span><input type="password" name="pass" id="password" ng-model="password" ng-required="true" ng-minlength="8" ng-pattern="/^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)[a-zA-Z\d]{8,}$/"><br>
          ...
          <font size="5px" color="green">New to site?
            <a href="./signup.php" style="color:red;text-decoration:none"> Create Account </a>
          </font>
        </form>
      </div>
    </div>
      ...
      if (empty($mob_err) && empty($password_err)) {
        ...
        if (mysqli_num_rows($rel) == 1) 
        {
          ...
          echo "<script>console.log('4' );</script>";
            ...
            ?>
              <script>
                window.location = "../farmer/farmer_profile.php";
              </script><?php
                      } else if ($_SESSION["type"] == "seller") {
                        ?><script>
                window.location = "../seller/seller_profile.php";
                        ...
                        ?><script>
                window.location = "../whole_seller/wseller_profile.php";
              </script><?php
                      } else {
                        ?><script>
                window.location = "../advertiser/advertiser_profile.php";
              </script><?php
                      }
                    } else {
